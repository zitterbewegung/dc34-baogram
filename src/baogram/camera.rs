...
//! quantize the result to full-resolution Mono1.

use baogram_core::image::{IMAGE_PIXELS, Mono1Image};
...
/// Freeze the next frame, read all 61,440 grayscale bytes, and quantize to
/// full-resolution (256x240) Mono1. The camera is powered down by the
/// freeze on the server side. The raw grayscale buffer is dropped as soon
/// as quantization completes.
pub fn capture_mono1(gfx: &Gfx) -> Result<(Mono1Image, CaptureMetrics), &'static str> {
    ...
    let image = Mono1Image::quantize(&frame, None).map_err(|_| "quantize failed")?;
    let t3 = tt.elapsed_ms();
    // `frame` (61,440 bytes) drops here; only the 7,680-byte Mono1 remains
